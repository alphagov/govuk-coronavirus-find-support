RSpec.describe "feel-safe" do
  describe "GET /feel-safe" do
    let(:selected_option) { I18n.t("coronavirus_form.questions.feel_safe.options").sample }

    context "without session data" do
      it "shows the form" do
        visit feel_safe_path

        expect(page.body).to have_content(I18n.t("coronavirus_form.questions.feel_safe.title"))
        I18n.t("coronavirus_form.questions.feel_safe.options").each do |option|
  ...
  describe "POST /feel-safe" do
    ...
    it "shows an error when no radio button selected" do
      ...
      expect(response.body).to have_content(I18n.t("coronavirus_form.questions.feel_safe.title"))
      expect(response.body).to have_content(I18n.t("coronavirus_form.errors.radio_field", field: I18n.t("coronavirus_form.questions.feel_safe.title").downcase))
    end
  end
end
