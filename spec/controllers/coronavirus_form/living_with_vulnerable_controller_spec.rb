...
RSpec.describe CoronavirusForm::LivingWithVulnerableController, type: :controller do
  describe "POST" do
    context "if living with vulnerable is the last question" do
      before do
        allow_any_instance_of(QuestionsHelper).to receive(:questions_to_ask).and_return(%w[living_with_vulnerable])
      ...
      it "saves the form response to the database" do
        ...
        expect(FormResponse.first.form_response).to eq(
          "questions_to_ask" => %w[living_with_vulnerable],
          "living_with_vulnerable" => I18n.t("coronavirus_form.groups.going_in_to_work.questions.living_with_vulnerable.options.option_yes.label"),
        )
